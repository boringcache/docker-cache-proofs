...
| `dependabot-go-modules` | Same Dependabot smoke issue, focused on the `go_modules` ecosystem image. This case measures the ecosystem Dockerfile surface; the full upstream `script/build` path also builds `Dockerfile.updater-core` first, covered by `dependabot-updater-core`. | [Recent smoke run](https://github.com/dependabot/dependabot-core/actions/runs/27011845235); BoringCache proof runs the upstream `go_modules/Dockerfile` in this repo. |
...
- `case_id=dependabot-updater-core` and `case_id=dependabot-go-modules` for the Dependabot smoke-image proof lanes.
