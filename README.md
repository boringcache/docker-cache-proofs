...
| `hoppscotch-backend-arm64` | Observed tax plus QEMU migration | The official ARM64 job runs on an x64 `ubuntu-latest` host through QEMU and took 67m25s; its backend cache export alone took 246.3 seconds. | [Official emulated ARM64 job](https://github.com/hoppscotch/hoppscotch/actions/runs/29410411043/job/87335877176); the proof runs that upstream backend target natively on the free `ubuntu-24.04-arm` runner with QEMU disabled. |
...
| `openstatus-dashboard` | Observed tax | Current dashboard cache exports took 206–364 seconds; the six-image matrix consumed 1,095 aggregate export seconds in one push. | [Official six-image application build](https://github.com/openstatusHQ/openstatus/actions/runs/29770045286); the proof preserves the upstream dashboard Dockerfile and two-platform output. |
| `openstatus-dashboard-arm64` | Native ARM64 proof | Uses the same dashboard seed and rolling commit while isolating `linux/arm64` on the free native ARM runner, with QEMU disabled. | Paired against the same GHA cache backend; the upstream two-platform result remains the public pain signal. |
| `karakeep-aio` | Reported optimization | Native AMD64 image exports repeatedly took 147–165 seconds, and the maintainers have already worked on Docker cache structure. | [Improve Docker caching](https://github.com/karakeep-app/karakeep/pull/2140); the proof isolates the full AIO target on the upstream native AMD64 path. |
| `anythingllm-primary` | Observed tax | Seven recent exports averaged 122.5 seconds and reached 166.8 seconds. | [Official primary image build](https://github.com/Mintplex-Labs/anything-llm/actions/runs/29620753615); the proof preserves its ARM-hosted two-platform build, SBOM, provenance, and QEMU requirement. |
| `anythingllm-primary-arm64` | Native ARM64 proof | Uses the same primary-image seed and rolling commit while isolating `linux/arm64` on the free native ARM runner, preserving SBOM and provenance with QEMU disabled. | Paired against the same GHA cache backend; the upstream two-platform result remains the public pain signal. |
| `misskey-develop-amd64` | Observed tax | Seven recent AMD64 exports averaged 120.1 seconds and reached 192.7 seconds during a high-cadence develop branch burst. | [Official develop image build](https://github.com/misskey-dev/misskey/actions/runs/29795853045); the proof isolates the upstream native AMD64 platform from its emulated ARM64 sibling. |
| `omninode-runtime` | Reported pain | The project measured 15–17 minutes in GHA cache export and raised its job timeout from 20 to 45 minutes after 23 consecutive cancellations. | [Documented timeout fix](https://github.com/OmniNode-ai/omnibase_infra/pull/567); the proof uses the public release-mode runtime Dockerfile on AMD64. |
| `vellum-assistant` | Reported pain | Cache export blocked an otherwise successful image release on three consecutive attempts. | [Non-fatal export fix](https://github.com/vellum-ai/vellum-assistant/pull/34998); the proof uses the assistant image on the same 8-core AMD64 runner class. Current export time is modest, so this is primarily a reliability case. |
