...
|---:|---|---|---|---|---|---|
| 1 | `aranya-rust` | Rust/sccache | `aranya-project/aranya` maintainers | [`aranya#135`](https://github.com/aranya-project/aranya/issues/135), [same-commit official unit-test run](https://github.com/aranya-project/aranya/actions/runs/26174641840), [same-commit BoringCache proof](https://github.com/boringcache/docker-cache-proofs/actions/runs/26965167907), [rolling2 proof](https://github.com/boringcache/docker-cache-proofs/actions/runs/26961876589) | Open issue; maintainers say GHA sccache is not ideal and discuss S3-backed sccache. Same source commit `577b2193...`: official GHA-backed sccache had 36.8% total hits, 24.0% Rust hits, 869 misses, and 413 write errors; BoringCache had 100% hits, 0 misses, and 0 cache errors/timeouts. The command still takes about 12m because non-cacheable/test work remains. | Pitch-ready with caveat: lead with compiler-cache misses going to zero, then ask whether the remaining tail is tests/runtime or another cacheable layer. |
| 2 | TODO `josh-rust-container` | Rust/sccache inside build container | `josh-project/josh` / Christian Schilling | [`josh#2025`](https://github.com/josh-project/josh/pull/2025), [recent Rust run](https://github.com/josh-project/josh/actions/runs/26955558243) | PR merged 2026-06-04; pain is very current. Their build containers run with `--network none`, so they added an `aws-sigv4-proxy` sidecar on a shared Podman network for R2 sccache without exposing credentials. The PR sets `network="sidecar"` on Rust build/test workspaces and injects `SCCACHE_ENDPOINT=http://{SIDECAR_IP}:3000` into the build container. Latest observed successful Rust run was about 70m, with many adjacent failures. | Add a dedicated container-side proof, not the generic host Rust lane: build container can reach only the BoringCache proxy/sidecar, no direct internet or credentials. This should also inform the Docker `--tool-cache` story for Rust inside containers. Local `podman` is not installed in this Mac workspace, so run faithful tests in CI or after Podman setup. |
| 3 | `tiny-congress-rust` | Rust/sccache | `icook/tiny-congress` | [`tiny-congress#683`](https://github.com/icook/tiny-congress/pull/683), [recent CI run](https://github.com/icook/tiny-congress/actions/runs/24946974229), [fresh proof](https://github.com/boringcache/docker-cache-proofs/actions/runs/26958655353), [rolling2 proof](https://github.com/boringcache/docker-cache-proofs/actions/runs/26959859541) | The pain was real, but the team already added ARC parallelism plus Garage S3-backed sccache. Recent CI is about 7-8m. BoringCache reaches 100% native-tool hit rate on rolling1/rolling2 with 0 cache errors/timeouts. | Reference proof, not first-wave outreach. Useful for showing BoringCache can replace bespoke S3/Garage sccache glue. |
...
Completed Rust/sccache proof table:

| Proof | Run | Result | Product read |
|---|---|---:|---|
| Aranya fresh `main` | [`26958655351`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26958655351) | 1097s cold command; 0.1% hit rate | Cold population only; corrected `boringcache/docker-cache-proof` workspace saves successfully. |
| Aranya rolling `main` same-commit | [`26965167907`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26965167907) | 733s command; 100% hit rate; 1295 hits / 0 misses | Same source commit as official Aranya run `26174641840`; clean cache-health comparison against GHA-backed sccache. |
| Aranya rolling1 | [`26961030438`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26961030438) | 818s command; 91.6% hit rate; 1186 hits / 109 misses | Good partial reuse on first real rolling commit. |
| Aranya rolling2 | [`26961876589`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26961876589) | 738s command; 100% hit rate; 1295 hits / 0 misses | Adapter is ready; remaining wall time is outside compiler misses. |
| Tiny Congress fresh `main` | [`26958655353`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26958655353) | 364s cold command; 0% hit rate | Cold population only; setup now builds `tc-postgres:local` from the source tree. |
| Tiny Congress rolling1 | [`26959544517`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26959544517) | 282s command; 100% hit rate; 419 hits / 0 misses | Compiler cache fully hot. |
| Tiny Congress rolling2 | [`26959859541`](https://github.com/boringcache/docker-cache-proofs/actions/runs/26959859541) | 252s command; 100% hit rate; 419 hits / 0 misses | Confirms repeatability; best as workaround-replacement proof. |
...
| Vercel AI | Turbo | [`vercel/ai#15393`](https://github.com/vercel/ai/issues/15393) reports Turbo CI test jobs with 0/111 hits and about 6.5m rebuild/retest per matrix job. | Real pain, but Vercel owns Turborepo. Keep as owner/internal validation and content signal, not a normal outreach or onboarding prospect. |
