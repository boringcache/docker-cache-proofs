...
host_tmp_root="${BENCHMARK_HOST_TMPDIR:-${TMPDIR:-/tmp}}"
mkdir -p "$host_tmp_root"
build_log="$(mktemp "${host_tmp_root}/boringcache-native-build.XXXXXX")"
native_tool_evidence_dir="$(mktemp -d "${host_tmp_root}/boringcache-native-tool.XXXXXX")"
...
buildctl_dir="$(mktemp -d "${host_tmp_root}/boringcache-buildctl.XXXXXX")"
